...
interface BaseRowViewSetter<E, RVH> {
    ...
    /**
     * This method should return Row object which contains 1) Row View
     ...
     *
     * @param parent The parent view which is passed by android.widget.BaseAdapter
     *               during call to
    ...
    /**
     * Set the View values by using passed data and view-holder object,essentially
     * this method is also responsible for setting up View onClickListeners for
     * its child Views.
     *
     * @param data          Data for given row position, the data present at given
     *                      position is retrieved by library and passed to subclasses so
     ...
     *
     * @param childViewsClickHandler This class enables you to register children view present in
     *                               your row by registering only single {@link OnClickListener}
     *                               listener for all children click events, and at runtime it will
     *                               be determined for which row and which child event occurred, for
     ...
     * registerChildrenViewClickEvents
     * this method will be called and user get few input params like,
     *
     * @param clickedChildView Registered child-view which was clicked.
     * @param rowData          Data for given row position, the data present at given
     *                         position is retrieved by library and passed to view-setter class
     *                         so that, library users do NOT have to look for their data on
     *                         child-view click events.
     * @param eventId          Event Id for clicked view, Pass different Event ID for all
